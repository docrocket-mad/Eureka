...
(function() {
  ...
  var CONFIG = {
    ...
    // Fallback endpoint if local server returns 500 (e.g. missing env vars)
    fallbackUrl: script && script.dataset.fallbackUrl || 'https://artificer.systems/api/voice-bug-report',
    accentColor: script && script.dataset.accentColor || '#7c3aed',
    // Hide the floating button — use window.openBugReporter() from your own UI instead
    hideButton: script && script.dataset.hideButton === 'true',
  };
  ...
  function init() {
    injectStyles();
    if (!CONFIG.hideButton) injectButton();
    injectOverlay();
  }
  ...
  function toggleRecord() {
    ...
    recognition.onresult = function(event) {
      var textarea = document.getElementById('abrTranscript');
  ...
  function submit() {
    ...
    function tryEndpoint(url, isRetry) {
      return fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: payload,
      }).then(function(resp) {
        // If local endpoint fails and we have a fallback, try it
        if (!resp.ok && !isRetry && CONFIG.fallbackUrl && url !== CONFIG.fallbackUrl) {
          console.warn('[BugReporter] Local endpoint ' + url + ' returned ' + resp.status + ', trying fallback');
          return tryEndpoint(CONFIG.fallbackUrl, true);
        }
    ...
    tryEndpoint(CONFIG.apiUrl, false)
    .then(function(data) {
      if (data.bugs && data.bugs.length > 0) {
        showResults(data.bugs, data.tickets || [], data.submitted, data.submit_errors);
      } else if (data.error) {
  ...
  function showResults(bugs, tickets, submitted, submitErrors) {
    ...
    var statusEl = document.getElementById('abrStatus');

    if (submitted && tickets.length > 0) {
      statusEl.textContent =
        '\u2705 ' + count + (count === 1 ? ' ticket' : ' tickets') + ' submitted! (' + ticketIds + ')';
    } else if (submitErrors && submitErrors.length > 0) {
      statusEl.innerHTML = '\u26A0\uFE0F Parsed ' + count + (count === 1 ? ' bug' : ' bugs') +
        ' but could not submit. ' + escapeHtml(submitErrors[0]);
      statusEl.style.color = '#ef4444';
    } else {
      statusEl.textContent = 'Parsed ' + count + (count === 1 ? ' bug' : ' bugs') + ':';
    }
  ...
  // Public opener — call from menu items, buttons, etc.
  window.openBugReporter = openOverlay;
