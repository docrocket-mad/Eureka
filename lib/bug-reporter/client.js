...
(function() {
  ...
  async function processRecording(blob) {
    ...
    try {
      ...
      if (data.parsed) {
        showResult(data.parsed, data.submitted, data.ticket_id);
      } else {
  ...
  function showResult(parsed, submitted, ticketId) {
    const preview = document.getElementById('abrPreview');
    preview.style.display = 'block';

    if (submitted) {
      document.getElementById('abrStatus').textContent = '\u2705 Ticket #' + (ticketId || '') + ' submitted!';
    } else {
      document.getElementById('abrStatus').textContent = 'Parsed your report (submission pending):';
    }

    preview.innerHTML = `
      <div class="abr-preview">
        <h3>${escapeHtml(parsed.title || 'Bug Report')}</h3>
        <div class="field"><div class="label">What happened</div><div class="value">${escapeHtml(parsed.what_happened || '')}</div></div>
        <div class="field"><div class="label">Expected</div><div class="value">${escapeHtml(parsed.expected || '')}</div></div>
        ${parsed.steps ? `<div class="field"><div class="label">Steps</div><div class="value">${escapeHtml(parsed.steps)}</div></div>` : ''}
        <div class="field"><div class="label">Severity</div><div class="value">${escapeHtml(parsed.severity || 'medium')}</div></div>
      </div>
    `;
