...
module.exports = function createBugReporter(config = {}) {
  ...
  // Log config at startup so missing keys are visible in Railway logs
  console.log(`[BugReporter:${source}] Config: webhookKey=${webhookKey ? 'SET' : 'MISSING'}, anthropicKey=${anthropicKey ? 'SET' : 'MISSING'}`);
  if (!webhookKey) {
    console.warn(`[BugReporter:${source}] WARNING: No webhook key configured. Set WEBHOOK_SECRET (or ARTIFICER_WEBHOOK_KEY / PIANO_OS_WEBHOOK_KEY) env var on this service to match Artificer's WEBHOOK_SECRET. Tickets will NOT be submitted.`);
  }

  const router = express.Router();

  // CORS for cross-origin clients (platforms without their own config)
  router.use('/api/voice-bug-report', (req, res, next) => {
